#!/usr/bin/env python3
"""
FeatureBuilderAdvanced (v100, domain-aware, NaN/column-safe)

- Handles Freddie Mac sentinels (9999, 999, '9'/'99') consistently.
- Adds domain-informed static risk features:
    * feat_credit_ltv_ratio
    * feat_dti_ltv_ratio
    * feat_upb_ltv_ratio
    * feat_payment_burden
    * feat_rate_to_term
- Temporal engineering + amortization signals (as before).
- Train-only fit (compliant with competition rules).
- Robust transform:
    * If a static column (including *_missing) existed in TRAIN but not in VALID/TEST,
      it is created as all zeros so you never get KeyError.

API:
    fb = FeatureBuilderAdvanced(use_pca=True, pca_comps=80)
    fb.fit(train_df)
    X_scaled, slices, X_embed = fb.transform(df_any)
"""

import numpy as np
import pandas as pd
from typing import Dict, List, Tuple, Optional
from sklearn.preprocessing import StandardScaler, RobustScaler, LabelEncoder
from sklearn.decomposition import PCA


# ---------- utilities ----------
def safe_div(a, b, eps=1e-9):
    return a / (b + eps)


def pct_change(a, axis=1, eps=1e-9):
    return (a[:, 1:] - a[:, :-1]) / (np.abs(a[:, :-1]) + eps)


# ---------- feature builder ----------
class FeatureBuilderAdvanced:
    """
    Train-only feature builder:
      - Sentinel handling -> NaN + missing flags (static)
      - Label-encode categorical with UNKNOWN (train-only fit)
      - Temporal engineering on multiple windows (trend/vol/first-diff stats)
      - Amortization residual-style signals from panel data
      - ONE global scaler (train-only) across final tabular feature matrix
      - Optional PCA (train-only) to provide an embedding for LOF/KMeans/etc.

    transform(...) returns:
      X_scaled  : scaled full feature matrix
      slices    : dict with slices {static, temporal, amort}
      X_embed   : PCA embedding (or X_scaled if PCA disabled)
    """

    def __init__(
        self,
        keep_month_idx_main=(0, 3, 6, 9, 12),
        keep_month_idx_alt1=(0, 2, 4, 6, 8, 10, 12),
        keep_month_idx_alt2=(0, 3, 6, 9),
        use_pca=True,
        pca_comps=80,
    ):
        # temporal window config
        self.keep_month_idx_main = set(keep_month_idx_main)
        self.keep_month_idx_alt1 = set(keep_month_idx_alt1)
        self.keep_month_idx_alt2 = set(keep_month_idx_alt2)
        self.use_pca = use_pca
        self.pca_comps = pca_comps

        self.cat_encoders: Dict[str, LabelEncoder] = {}
        self.impute_vals: Dict[str, float] = {}
        self.scaler: Optional[StandardScaler] = None
        self.pca: Optional[PCA] = None

        self.static_cols: List[str] = []
        self.temporal_cols_all: List[str] = []
        self._fitted = False

        self._amort_slice = slice(0, 0)
        self._temporal_slice = slice(0, 0)

        # for debugging / meta-learning
        self._static_feat_names: List[str] = []
        self._temporal_feat_names: List[str] = []
        self._amort_feat_names: List[str] = []
        self._feature_names_full: List[str] = []

    # ---------- helpers ----------
    @staticmethod
    def _is_temporal(col: str) -> bool:
        if "_" not in col:
            return False
        left = col.split("_", 1)[0]
        return left.isdigit()

    def _sentinel_map(self, df: pd.DataFrame) -> pd.DataFrame:
        """
        Map known sentinel codes to NaN and add missing flags for some statics.
        Based on Freddie Mac documentation.
        """
        df = df.copy()

        # Numeric sentinel codes -> NaN + missing flags
        SENT = {
            "CreditScore": 9999,
            "OriginalDTI": 999,
            "OriginalLTV": 999,
            "MI_Pct": 999,
            "EstimatedLTV": 999,
        }
        for col, bad in SENT.items():
            if col in df.columns:
                miss = (df[col] == bad)
                if miss.any():
                    df.loc[miss, col] = np.nan
                    df[f"{col}_missing"] = miss.astype(int)

        # Flags '9'/'99' => missing for specific categoricals
        FLAG_9 = [
            "FirstTimeHomebuyerFlag",
            "OccupancyStatus",
            "LoanPurpose",
            "Channel",
            "PropertyType",
            "ProgramIndicator",
        ]
        for col in FLAG_9:
            if col in df.columns:
                x = df[col].astype(str).str.strip()
                miss = (x == "9") | (x == "99")
                if miss.any():
                    df.loc[miss, col] = np.nan
                    df[f"{col}_missing"] = miss.astype(int)

        return df

    def _add_static_risk_features(self, df: pd.DataFrame) -> pd.DataFrame:
        """
        Add a small set of domain-informed static risk features.
        Uses only per-loan info (no label leakage, no valid stats).
        """
        df = df.copy()

        def col(name):
            return df[name].astype(float) if name in df.columns else None

        cs = col("CreditScore")
        dti = col("OriginalDTI")
        ltv = col("OriginalLTV")
        upb = col("OriginalUPB")
        rate = col("OriginalInterestRate")
        term = col("OriginalLoanTerm")

        # 1) Credit–LTV ratio
        if cs is not None and ltv is not None:
            df["feat_credit_ltv_ratio"] = cs / (np.abs(ltv) + 1.0)

        # 2) DTI–LTV ratio
        if dti is not None and ltv is not None:
            df["feat_dti_ltv_ratio"] = dti / (np.abs(ltv) + 1.0)

        # 3) UPB–LTV ratio
        if upb is not None and ltv is not None:
            df["feat_upb_ltv_ratio"] = upb / (np.abs(ltv) + 1.0)

        # 4) Monthly payment burden
        if upb is not None and rate is not None:
            monthly_rate = rate / 100.0 / 12.0
            df["feat_payment_burden"] = upb * monthly_rate

        # 5) Rate-to-term
        if rate is not None and term is not None:
            df["feat_rate_to_term"] = rate / (np.abs(term) + 1.0)

        return df

    def _collect_columns(self, df: pd.DataFrame):
        self.static_cols = []
        self.temporal_cols_all = []
        for c in df.columns:
            if c in ("index", "Id", "target"):
                continue
            if self._is_temporal(c):
                self.temporal_cols_all.append(c)
            else:
                self.static_cols.append(c)

    def _fit_cat_and_impute(self, df: pd.DataFrame):
        # Fit label encoders for categorical static columns
        for c in self.static_cols:
            if df[c].dtype == "object":
                enc = LabelEncoder()
                vals = df[c].fillna("MISSING").astype(str).unique().tolist()
                if "UNKNOWN" not in vals:
                    vals.append("UNKNOWN")
                enc.fit(vals)
                self.cat_encoders[c] = enc

        # Fit numeric imputation values
        for c in self.static_cols:
            if c not in self.cat_encoders:
                v = df[c].dropna()
                self.impute_vals[c] = float(v.median() if len(v) else 0.0)

    def _transform_static(self, df: pd.DataFrame) -> pd.DataFrame:
        """
        Robust static transform:
        - If a static column used at train time is missing in df (e.g. *_missing flag
          never created because no sentinel), create it as zeros.
        """
        cols_dict = {}
        for c in self.static_cols:
            if c in df.columns:
                cols_dict[c] = df[c]
            else:
                # For missing flags / engineered columns, default to 0
                cols_dict[c] = pd.Series(0, index=df.index, dtype=float)

        data = pd.DataFrame(cols_dict, index=df.index)

        # Encode categoricals
        for c, enc in self.cat_encoders.items():
            if c not in data.columns:
                data[c] = pd.Series("UNKNOWN", index=df.index)
            x = data[c].fillna("MISSING").astype(str)
            x = x.where(x.isin(enc.classes_), "UNKNOWN")
            data[c] = enc.transform(x)

        # Impute numeric statics
        for c in self.static_cols:
            if c not in self.cat_encoders:
                data[c] = data[c].fillna(self.impute_vals.get(c, 0.0))

        return data

    def _temporal_block_multiwindow(self, df: pd.DataFrame) -> pd.DataFrame:
        """
        Temporal feature builder:
        - Group temporal columns by suffix feature type.
        - For each type, build trends/vol/first-diff stats across multiple windows.
        """
        by_type: Dict[str, List[str]] = {}
        for c in self.temporal_cols_all:
            if "_" in c:
                by_type.setdefault(c.split("_", 1)[1], []).append(c)
        if not by_type:
            return pd.DataFrame(index=df.index)

        feats = {}
        for ftype, cols in by_type.items():
            cols_sorted = sorted(cols, key=lambda x: int(x.split("_", 1)[0]))
            M = df[cols_sorted].to_numpy(float)

            # forward/backward fill per row to handle rare NaNs
            mask = np.isnan(M)
            idx = np.where(~mask, np.arange(M.shape[1]), 0)
            np.maximum.accumulate(idx, axis=1, out=idx)
            M = M[np.arange(M.shape[0])[:, None], idx]
            mask = np.isnan(M)
            idx2 = np.where(~mask, np.arange(M.shape[1]), M.shape[1] - 1)
            np.minimum.accumulate(idx2[:, ::-1], axis=1, out=idx2[:, ::-1])
            M = M[np.arange(M.shape[0])[:, None], idx2]
            M = np.nan_to_num(M, nan=0.0)

            # windows
            def select_steps(step_set):
                step_idx = []
                for c in cols_sorted:
                    m = int(c.split("_", 1)[0])
                    if m in step_set:
                        step_idx.append(cols_sorted.index(c))
                if not step_idx:
                    return None
                return M[:, step_idx]

            for name, step_set in [
                ("w_main", self.keep_month_idx_main),
                ("w_alt1", self.keep_month_idx_alt1),
                ("w_alt2", self.keep_month_idx_alt2),
            ]:
                A = select_steps(step_set)
                if A is None or A.shape[1] < 2:
                    continue
                first, last = A[:, 0], A[:, -1]
                trend = safe_div(last - first, np.abs(first) + 1.0)
                vol = safe_div(A.std(axis=1), np.abs(A.mean(axis=1)) + 1.0)
                # first differences stats
                d = pct_change(A)  # shape [n, T-1]
                d_mean = np.nanmean(d, axis=1)
                d_std = np.nanstd(d, axis=1)

                feats[f"{ftype}_{name}_trend"] = trend
                feats[f"{ftype}_{name}_vol"] = vol
                feats[f"{ftype}_{name}_dmean"] = d_mean
                feats[f"{ftype}_{name}_dstd"] = d_std

            # extra global trend signal
            first_all, last_all = M[:, 0], M[:, -1]
            g_trend = safe_div(last_all - first_all, np.abs(first_all) + 1.0)
            feats[f"{ftype}_global_trend"] = g_trend

        return pd.DataFrame(feats, index=df.index)

    @staticmethod
    def _annuity_payment_for_state(upb_prev, rt, rem_m):
        # rt is monthly rate (already divided by 12)
        rem_m = np.maximum(rem_m, 1.0)
        ok = (rt > 0) & (rem_m > 0)
        out = np.zeros_like(upb_prev)
        with np.errstate(over="ignore", invalid="ignore"):
            num = rt * np.power(1.0 + rt, rem_m)
            den = np.power(1.0 + rt, rem_m) - 1.0
            P = np.where((den > 0) & ok, upb_prev * (num / den), safe_div(upb_prev, rem_m))
        out = P
        return out

    def _amort_signals(self, df: pd.DataFrame) -> pd.DataFrame:
        """
        Amortization signals:
        - Uses InterestBearingUPB path & rate/term info.
        - Produces shortfall-based features + mask flag.
        """
        tcols = [c for c in self.temporal_cols_all if c.endswith("_InterestBearingUPB")]
        if not tcols:
            return pd.DataFrame(index=df.index)
        tcols_sorted = sorted(tcols, key=lambda x: int(x.split("_", 1)[0]))
        IB = df[tcols_sorted].to_numpy(float)

        # monthly rate
        rcols = [c for c in self.temporal_cols_all if c.endswith("_CurrentInterestRate")]
        if rcols:
            rcols_sorted = sorted(rcols, key=lambda x: int(x.split("_", 1)[0]))
            RT = df[rcols_sorted].to_numpy(float) / 1200.0
        else:
            if "OriginalInterestRate" not in df.columns:
                return pd.DataFrame(index=df.index)
            RT = np.tile((df["OriginalInterestRate"].to_numpy(float) / 1200.0)[:, None], (1, IB.shape[1]))

        # remaining months
        mcols = [c for c in self.temporal_cols_all if c.endswith("_RemainingMonthsToLegalMaturity")]
        if mcols:
            mcols_sorted = sorted(mcols, key=lambda x: int(x.split("_", 1)[0]))
            RM = df[mcols_sorted].to_numpy(float)
        else:
            if "OriginalLoanTerm" not in df.columns:
                return pd.DataFrame(index=df.index)
            T0 = df["OriginalLoanTerm"].to_numpy(float)
            RM = np.maximum(T0[:, None] - np.arange(IB.shape[1])[None, :], 1.0)

        # applicability mask
        is_frm = df.get("ProductType", pd.Series("FRM", index=df.index)).astype(str).str.upper().eq("FRM")
        is_io = df.get("InterestOnlyFlag", pd.Series("N", index=df.index)).astype(str).str.upper().eq("Y")
        is_bal = df.get("BalloonIndicator", pd.Series("N", index=df.index)).astype(str).str.upper().eq("Y")
        use_mask = (is_frm & (~is_io) & (~is_bal)).to_numpy()

        # expected principal reduction per step using annuity
        n = IB.shape[0]
        T = IB.shape[1]
        ExpPrin = np.zeros((n, T))
        for t in range(1, T):
            upb_prev = IB[:, t - 1]
            rt = RT[:, t]
            rem = RM[:, t]
            P = self._annuity_payment_for_state(upb_prev, rt, rem)
            exp_prin = np.maximum(P - rt * upb_prev, 0.0)
            ExpPrin[:, t] = exp_prin

        ObsPrin = np.maximum(IB[:, :-1] - IB[:, 1:], 0.0)
        E = ExpPrin[:, 1:]
        S = (E - ObsPrin) / (np.abs(E) + 1e-9)
        S = np.clip(S, 0.0, 1.0)

        short_mean = S.mean(axis=1)
        short_70 = (S > 0.70).mean(axis=1)
        short_50 = (S > 0.50).mean(axis=1)

        # mask non-applicable loans → 0, but expose mask flag
        short_mean = np.where(use_mask, short_mean, 0.0)
        short_70 = np.where(use_mask, short_70, 0.0)
        short_50 = np.where(use_mask, short_50, 0.0)

        return pd.DataFrame(
            {
                "amort_short_mean": short_mean,
                "amort_short_70": short_70,
                "amort_short_50": short_50,
                "amort_mask_not_applicable": (~use_mask).astype(int),
            },
            index=df.index,
        )

    # ---------- public API ----------
    def fit(self, df_train: pd.DataFrame):
        """
        Fit on TRAIN only (no valid/test). Complies with competition rules.
        """
        df = df_train.copy()
        df = self._sentinel_map(df)
        df = self._add_static_risk_features(df)

        self._collect_columns(df)
        self._fit_cat_and_impute(df)

        Xs = self._transform_static(df)
        Xt = self._temporal_block_multiwindow(df)
        Xa = self._amort_signals(pd.concat([df, Xt], axis=1))

        X_full = pd.concat([Xs, Xt, Xa], axis=1).astype(float)

        # remember feature names
        self._static_feat_names = list(Xs.columns)
        self._temporal_feat_names = list(Xt.columns)
        self._amort_feat_names = list(Xa.columns)
        self._feature_names_full = (
            self._static_feat_names + self._temporal_feat_names + self._amort_feat_names
        )

        #self.scaler = StandardScaler().fit(X_full.values)
        self.scaler = RobustScaler().fit(X_full.values)
        X_scaled = self.scaler.transform(X_full.values)

        if self.use_pca:
            k = min(self.pca_comps, X_scaled.shape[1])
            self.pca = PCA(n_components=k, random_state=42).fit(X_scaled)

        # remember slices
        start_temporal = Xs.shape[1]
        end_temporal = start_temporal + Xt.shape[1]
        start_amort = end_temporal
        end_amort = start_amort + Xa.shape[1]
        self._temporal_slice = slice(start_temporal, end_temporal)
        self._amort_slice = slice(start_amort, end_amort)

        self._fitted = True

    def transform(self, df_any: pd.DataFrame) -> Tuple[np.ndarray, Dict[str, slice], np.ndarray]:
        """
        Transform ANY split (train/valid/test) using only train-fitted objects.
        """
        assert self._fitted, "Call fit() first."
        df = df_any.copy()
        df = self._sentinel_map(df)
        df = self._add_static_risk_features(df)

        Xs = self._transform_static(df)
        Xt = self._temporal_block_multiwindow(df)
        Xa = self._amort_signals(pd.concat([df, Xt], axis=1))

        X_full = pd.concat([Xs, Xt, Xa], axis=1).astype(float)
        X_scaled = self.scaler.transform(X_full.values)
        X_embed = self.pca.transform(X_scaled) if self.use_pca else X_scaled

        slices = {
            "static": slice(0, Xs.shape[1]),
            "temporal": self._temporal_slice,
            "amort": self._amort_slice,
        }
        return X_scaled, slices, X_embed

    # Optional helper for debugging / analysis
    def get_feature_names(self) -> List[str]:
        """
        Returns the full list of feature names in the order of X_scaled columns.
        Only valid after fit().
        """
        if not self._fitted or not self._feature_names_full:
            raise RuntimeError("FeatureBuilderAdvanced is not fitted yet.")
        return list(self._feature_names_full)
