...
# =============================
# Script 4 of 4: pca_reconstruction_baseline.py
# =============================
#!/usr/bin/env python3
"""
...
def main():
    ...
    err=np.mean((Xvho-Xh)**2, axis=1)
    # normalize
    err=(err-err.min())/(err.max()-err.min()+1e-12)
    auroc=roc_auc_score(vho['target'], err)
    auprc=average_precision_score(vho['target'], err)
    print('[PCA-Rec] Holdout AUROC=%.4f AUPRC=%.4f'%(auroc,auprc))
