#!/usr/bin/env python3
# Ultra ensemble V2 (unsupervised, compliant):
...
from feature_builder_advanced_v2 import FeatureBuilderAdvancedV2
...
def mahal_score_train(Xtr: np.ndarray) -> Tuple[np.ndarray, np.ndarray]: # Returns (mean, inv_cov)
    # Robust covariance for Mahalanobis distance
    from sklearn.covariance import MinCovDet # More robust than EllipticEnvelope
    robust_cov = MinCovDet(random_state=42).fit(Xtr)
    mu = robust_cov.location_
    inv_cov = robust_cov.get_precision()
    return mu, inv_cov

def mahal_score(X: np.ndarray, mu: np.ndarray, inv_cov: np.ndarray) -> np.ndarray:
    diff = X - mu
    md = np.sqrt(np.sum(diff @ inv_cov * diff, axis=1))
    return md

def gmm_train(Xtr: np.ndarray, n_components=8, rng=42) -> GaussianMixture:
    # Fit a GMM on PCA space for density-based scoring
    gmm = GaussianMixture(n_components=n_components, random_state=rng, n_init=10)
    gmm.fit(Xtr)
    return gmm

def gmm_score(gmm: GaussianMixture, X: np.ndarray) -> np.ndarray:
...
def fuse_candidates_prob(prob_map_valid: Dict[str, np.ndarray], weights: Dict[str, float]) -> Dict[str, np.ndarray]:
    ...
    # Product of (1 - prob) -> low if any is high
    cand["p_prod_inv"] = np.prod(1.0 - P, axis=1)
    return cand

# ---------- main ----------
def main():
    print("=== Ultra Unsupervised Ensemble V2 (Compliant) ===")
    ...
    pca_settings = [60, 80, 100, 120] # Potentially better for the new features
    ...
    for pca_k in pca_settings:
        ...
        fb = FeatureBuilderAdvancedV2(use_pca=True, pca_comps=pca_k)
        fb.fit(train)
        ...
        # LOF multi-k
        lof_ks = [5,6,7,8,10,12,15] # Slightly larger k range
        ...
        km = KMeans(n_clusters=15, n_init=10, random_state=42).fit(Xtr) # More clusters
        ...
        for c in range(km.n_clusters):
            ...
            for k in (6,7,8,9): # Slightly different k range for cluster LOF
        ...
        # k-distance multi-k
        kd_ks = [4,6,8,10,12] # Slightly different k range
        ...
        if_model = iforest_train(Xtr, n_estimators=600, max_samples=1.0, rng=42) # Slightly more estimators
        ...
        # Mahalanobis Distance (on scaled features, using robust covariance)
        try:
            mahal_mu, mahal_inv_cov = mahal_score_train(Xtr_scaled) # Use scaled features for Mahal
            detectors_scores_valid["mahal"] = mahal_score(Xv_scaled, mahal_mu, mahal_inv_cov)
            detectors_scores_test["mahal"]  = mahal_score(Xt_scaled, mahal_mu, mahal_inv_cov)
            detectors_scores_train["mahal"] = mahal_score(Xtr_scaled, mahal_mu, mahal_inv_cov)
        except Exception as e:
            print(f"Mahalanobis failed: {e}")

        # GMM Density (on PCA space)
        try:
            gmm_model = gmm_train(Xtr, n_components=10, rng=42) # More components
            detectors_scores_valid["gmm"] = gmm_score(gmm_model, Xv)
            detectors_scores_test["gmm"]  = gmm_score(gmm_model, Xt)
            detectors_scores_train["gmm"] = gmm_score(gmm_model, Xtr)
        except Exception as e:
            print(f"GMM failed: {e}")

        # Random-Projection LOF bagging (MaxRank within bags)
        detectors_scores_valid["rp_lof"] = random_proj_lof_scores(Xtr, Xv, B=50, dim=min(70, Xtr.shape[1]), k=7, seed=42) # More bags, slightly higher dim
        detectors_scores_test["rp_lof"]  = random_proj_lof_scores(Xtr, Xt, B=50, dim=min(70, Xtr.shape[1]), k=7, seed=43)
        detectors_scores_train["rp_lof"] = random_proj_lof_scores(Xtr, Xtr, B=50, dim=min(70, Xtr.shape[1]), k=7, seed=41)
        ...
        # advanced temporal features (from scaled full features)
        adv_temp_slice = sl_v["advanced_temporal"]
        if adv_temp_slice.stop - adv_temp_slice.start > 0:
            def adv_temp_sc(X_scaled):
                block = X_scaled[:, np.r_[adv_temp_slice.start:adv_temp_slice.stop]]
                # Aggregate some advanced temporal signals, e.g., max or mean of variance/std features
                # Example: mean of local variance features if they exist
                var_cols = [i for i, col in enumerate(fb._advanced_temporal_cols) if 'var' in col]
                if var_cols:
                    return np.mean(block[:, var_cols], axis=1)
                else:
                    # Fallback: mean of all advanced temporal features
                    return np.mean(block, axis=1)
            # Note: fb doesn't store column names for advanced temporal features directly in slices dict
            # So we use a simpler aggregation for now.
            # A more robust way would be to store column names in the builder itself.
            # For simplicity, just aggregate all in the slice.
            detectors_scores_valid["adv_temp"] = np.mean(Xv_scaled[:, adv_temp_slice.start:adv_temp_slice.stop], axis=1)
            detectors_scores_test["adv_temp"]  = np.mean(Xt_scaled[:, adv_temp_slice.start:adv_temp_slice.stop], axis=1)
            detectors_scores_train["adv_temp"] = np.mean(Xtr_scaled[:, adv_temp_slice.start:adv_temp_slice.stop], axis=1)
        ...
        keep = [k for k,_ in sorted_dets if per_ap[k] >= 0.15][:12] # Keep more detectors initially
        if not keep:
    ...
    sub.to_csv("SUB_ultra_unsup_v2.csv", index=False)
    print("Saved SUB_ultra_unsup_v2.csv")
